...
# data = 'techtc'
# data = 'gene'
save_dir_kcss = '{}_results'.format(data)
save_dir_unifrom = os.path.join(os.getcwd(), '../baselines/{}_uniform'.format(data))
...
plt_title_err = '{} L1 Error (Distributed)'.format(data_to_title[data])
plt_title_time = '{} Time (Distributed)'.format(data_to_title[data])
...
def load_target_results(results_no, target_ranks, method_name, save_dir, num_servers=None):
    all_errors = []
    all_times = []
    for rno in results_no:
        ...
        for rank_idx in range(len(target_ranks)):
            ...
            savefile = '{}_data_{}_servers_{}_rank_{}'\
                    .format(method_name, data, num_servers, rank)
...
# should be of size: # results x 8
regular_errs, regular_times = load_target_results(results_no, target_ranks,
                                                  'regular', save_dir_kcss, num_servers)
greedy_errs, greedy_times = load_target_results(results_no, target_ranks,
                                                'greedy', save_dir_kcss, num_servers)
uniform_errs, uniform_times = load_target_results(results_no, target_ranks,
                                                  'uniform', save_dir_unifrom, num_servers)
...
uniform_errs_mean = np.mean(uniform_errs / A_1norm, axis=0)
uniform_errs_std = np.std(uniform_errs / A_1norm, axis=0)
...
uniform_times_mean = np.mean(uniform_times, axis=0)
uniform_times_std = np.std(uniform_times, axis=0)
...
# baselines
ax1.scatter(target_ranks, uniform_errs_mean, marker='^', s=mk_size)
ax1.errorbar(target_ranks, uniform_errs_mean,
            yerr=uniform_errs_std,
...
# baselines
ax2.scatter(target_ranks, uniform_times_mean, marker='^', s=mk_size)
ax2.errorbar(target_ranks, uniform_times_mean,
            yerr=uniform_times_std,
            label='uniform', alpha=0.5)
...
# plt.show()
plt.savefig('{}_distributed'.format(data), bbox_inches='tight',
    pad_inches=0.02)
